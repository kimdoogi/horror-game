...
namespace HorrorGame.EditorTools
{
    ...
    /// this pass is testable — the ghost state is entered, the cooldown is 45 s, there is
    /// no path to a voice channel or to the exit — and none of them touches the question
    /// §09 actually asks, which is 죽으면 지루하다 / 볼 게 있고 할 게 있다. A dead player
    /// flying through an unlit five-storey basement with nothing on screen is a passing
    /// test and a black rectangle. STATUS.md §4.3 measures the building's zone views at a
    /// median of 4.2–8.4 of 255 with the practicals off, so that outcome is the default
    /// and not a hypothetical.
    /// </para>
    /// <para>
    /// So this walks the real path: begin a match, pick up a §08 전리품, get caught
    /// through <c>MatchDirector</c>'s own grab, and photograph the frames that follow —
    /// at the body, beside a 물건 with the verb armed, up through the storeys, and over
    /// §06's monster. It writes an <c>_asplayed</c> and a <c>_nograde</c> exposure of the
    /// same frame so <see cref="GhostViewGrade"/>'s stops can be judged rather than
    /// assumed: <c>tools/render/frame_stats.py</c> reads both against ART.md's bands.
    /// </para>
    /// <para>
    ...
    public static class GhostSeatShot
    {
        ...
        public static string Capture(string tag, bool sweepExposure)
        {
            ...
            report.AppendLine("  died at       " + Fmt(ghost.DeathPosition.ToVector3())
                + ", 전리품 " + ghost.OwnDroppedLootValue.ToString(CultureInfo.InvariantCulture) + " 크레딧"
            ...
            // 10 — turned round to look at its own corpse and its own pile (§08 → §09).
            LookAt(ghosts, camera, ghost.DeathPosition.ToVector3() + (Vector3.up * 0.9f), 3.5f);
            Shoot(camera, root, tag, "10_my_own_body", report,
                "§08's pile, seen and unsayable — "
                + ghost.OwnDroppedLootValue.ToString(CultureInfo.InvariantCulture) + " 크레딧");

            // 20 — beside a 물건, verb armed. §09's whole outbound bandwidth.
            //
            // Flown to rather than found underfoot: §12's corridors are largely bare
            // (STATUS.md §3.15) so where a player happens to fall very often has nothing
            // rattleable in it — and a ghost's answer to that is to go somewhere that
            // does, which is the flight this frame is supposed to show.
            var thing = NearestThing(ghost.Position.ToVector3());
            if (thing != null)
            {
                LookAt(ghosts, camera, thing.transform.position, RattleStandOffMetres);

                // Searched explicitly. GhostSession refreshes its target from Update, and
                // Update does not run outside play mode, so the cached one is from
                // wherever the ghost was standing a shot ago.
                var target = ghosts.LookForSomethingToShake();
                Shoot(camera, root, tag, "20_beside_a_thing", report,
                    target.Found
                        ? "[" + GhostSession.RattleKey + "] " + target.Label + " · "
                          + target.Distance.ToString("0.0", CultureInfo.InvariantCulture) + "m"
                        : "flew to " + thing.name + " and GhostRattleTarget found nothing ⚠");

                if (ghosts.TryRattle(out var rattle))
                {
                    Settle(ghosts);
                    Shoot(camera, root, tag, "25_just_rattled", report,
                        "§09 신호 landed; the next one is "
                        + GameConstants.GhostRattleCooldownSeconds.ToString("0", CultureInfo.InvariantCulture)
                        + " s away");
                }
                else
                {
                    report.AppendLine("  25_just_rattled       skipped — " + rattle.Failure);
                }
            }
            else
            {
                report.AppendLine("  20_beside_a_thing     skipped — this map has no Interactable to shake");
            }
        ...
        /// ghost below ground, and <c>MatchState.TryKill</c> refuses on the surface.
        /// </summary>
        ...
        private static void Kill(MatchDirector director, PlayerMotor motor, MonsterAgent? monster)
        {
            ...
            for (var i = 0; i < KillSteps && monster.State != MonsterStateId.Chase; i++)
            {
                ...
                Teleport(motor.gameObject, bait);
        ...
        private static Vector3 InFrontOf(MonsterAgent monster)
        {
            ...
            for (var offset = 0f; offset <= 75f; offset += 15f)
            {
                foreach (var sign in new[] { 1f, -1f })
                {
                    ...
                    heading.Normalize();
                    if (Physics.Raycast(eye, heading, BaitDistanceMetres + 0.5f, ~0, QueryTriggerInteraction.Ignore))
                    {
                        continue;
                    }
        ...
        /// <summary>
        /// The nearest thing on this map §09 would call a 물건. An
        /// <see cref="Interactable"/> is what <see cref="GhostRattleTarget"/> prefers, so
        /// flying to one is flying to a frame where the verb is armed.
        /// </summary>
        private static Interactable? NearestThing(Vector3 from)
        {
        ...
        /// <summary>How far in front of the monster the bait stands, metres. Inside §06's cone and well inside its sight range.</summary>
        ...
        /// <summary>
        /// How far from a 물건 the rattle frame is taken, metres. Inside
        /// <c>GameConstants.GhostRattleRange</c> with room to spare, so the shot exercises
        /// the armed case rather than the boundary.
        /// </summary>
        private const float RattleStandOffMetres = 2f;
