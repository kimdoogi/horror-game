...
namespace HorrorGame.UI.Screens
{
    ...
    /// §07 hides the hour underground on purpose — "안에서는 시간 감각이 없다" — so a
    /// player has no way to check whether a paused game charged them, and the only
    /// honest answer is a line on the screen that made the promise.
    /// </para>
    /// <para>
    /// <b>It darkens the world behind it, which the shop deliberately does not.</b>
    /// §08's shop is the team standing in the open with the night advancing and must not
    /// feel like a safe box; this is the opposite state — nothing is advancing — and the
    /// screen should look like it.
    /// </para>
    /// </summary>
        ...
        public void Open(Action onResume, Action onSettings, Action onQuitToMenu)
        {
            ...
            if (_clockNote != null)
            {
                _clockNote.text = MatchPause.IsPaused
                    ? "§07 시계가 멈췄다. 위협도도, 괴물도, 배터리도 함께 멈춰 있다."
                    : "§07 시계를 멈추지 못했다 — 이 판은 계속 흐르고 있다.";
        ...
        protected override void Build(RectTransform root)
        {
            ...
            UiFactory.Place(
                (RectTransform)UiFactory.CreateText(
                    ...
                    "메뉴로 나가면 이 판은 사라진다. §02의 「생존」은 지상에서 걸어 나가는 것이지 여기서 나가는 것이 아니다.",
