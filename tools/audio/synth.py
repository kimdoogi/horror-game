...
Every sound the game ships is generated by code in this directory. Nothing is
sampled, downloaded, or licensed, which matters for a Steam release: there is no
attribution to track and no clip that turns out to be someone else's.

It also matters for the design. §12 makes floor material a *gameplay* channel —
the Listener locates the monster by what its footsteps land on, so the five
surfaces have to be reliably distinguishable by ear. Synthesis lets us tune that
distinguishability directly instead of hunting for sample packs that happen to
contrast well. `modal_impact` exists for exactly that job.
